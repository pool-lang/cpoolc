...
public:
    ...
    // Tries to read ahead n characters
    // if there are less than n chars remaining return as much read chars
    // as possible
    QString readAhead(int n);
    ...
    // returns the Token between the positions of two buffer states
    // inclusive. Asserts that for each buffer state p1 == p2
    QString getRange(const BufferState &s1,
                     const BufferState &s2,
                     int &line,
                     int &col,
                     int &pos) const;
    ...
    void restoreState(const BufferState &s);
private:
